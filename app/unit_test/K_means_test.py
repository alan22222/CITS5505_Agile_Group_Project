...
def check_column_input(df, column_input):
    """

    """
    if isinstance(column_input, (str, int)):
        column_input = [column_input]
    valid = True
    for col in column_input:
        if isinstance(col, str):
            if col not in df.columns:
                warnings.warn(f"Label '{col}' Does not exist in current data frame", UserWarning)
                valid = False
        elif isinstance(col, int):
            if col < 0 or col >= len(df.columns):
                warnings.warn(f"Label index {col} is not valid. {len(df.columns) - 1}）", UserWarning)
                valid = False
        else:
            warnings.warn(f"Invalid input type: {type(col)}", UserWarning)
            valid = False
    return valid
...
def kmeans_function(clean_content, type):
    ...
    try:
        ...
        if not check_column_input(df=clean_content,column_input=0):
            raise ValueError("Invalid label column, please check your input carefully")
